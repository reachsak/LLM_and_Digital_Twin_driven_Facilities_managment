    ...
    for day in range(num_days):
        for i in range(24 * 60):  # 24 hours * 60 minutes
            ...
            if timestamp.weekday() < 5:  # Weekday
                ...
                occupancy = random.randint(0, 10)
                if 10 <= hour < 18:  # Daytime hours
                    lux_intensity = round(random.uniform(100, 500), 2)
                else:  # Night hours
                    lux_intensity = round(random.uniform(0, 50), 2)
...
save_to_csv(synthetic_data, 'week_synthetic_sensor_data.csv')

print("7 days of synthetic dataset has been generated and saved to 'week_synthetic_sensor_data.csv'")